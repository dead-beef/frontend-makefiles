...
module.exports = class MakeVars {
	 ...
	 * @param   {string} pack  Module name.
	 * @returns {string}       Module main file path relative to this.root.
	 */
	resolve(pack) {
		...
		main = path.join(packRoot, packJson.main);
		return this.packMain[pack] = path.relative(this.root, main);
	}
